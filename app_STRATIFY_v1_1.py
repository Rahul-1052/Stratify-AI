...
def make_pdf_report(title, sections, metrics=None):
    ...
    # Clean, aligned brand banner for PDF only. Website header is untouched.
    logo_flowable = ""
    try:
        if APP_HORIZONTAL_LOGO_PATH:
            logo_flowable = RLImage(str(APP_HORIZONTAL_LOGO_PATH), width=1.05 * inch, height=0.32 * inch)
        elif APP_ICON_PATH:
            logo_flowable = RLImage(str(APP_ICON_PATH), width=0.36 * inch, height=0.36 * inch)
    except Exception:
        logo_flowable = ""

    brand_copy = [
        Paragraph("STRATIFY", brand_style),
    ...
    if logo_flowable:
        header = Table(
            ...
            colWidths=[1.35 * inch, page_width - 1.35 * inch],
    ...
    story.append(Paragraph(pdf_clean(title), title_style))
